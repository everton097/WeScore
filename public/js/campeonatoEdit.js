...
document
	.querySelector("#EditCampeonatoForm")
	.addEventListener("click", function () {
		// Obtenha o ID do time clicado
		const campeonatoId = document.querySelector("#createCampeonatoForm").dataset
			.campeonatoId;
		// Obter os dados do formulário de criação
		const formData = new FormData(
			document.querySelector("#createCampeonatoForm")
		);
		...
		if (validateForm(formData)) {
			...
			const file = formData.get("logoCampeonato");
			// Verificar se o campo logoCampeonato está vazio
			if (file.name) {
			...
			axios
				.put(`${url}campeonato/${campeonatoId}`, data, config)
				.then((response) => {
					Swal.fire({
						icon: "success",
						title: "Campeonato atualizado com sucesso",
						showConfirmButton: false,
						timer: 1500,
					}).then(() => {
						window.location.href = `/painelws/campeonato/`;
					});
				})
				.catch((error) => {
					console.error(error);
