...
document
	...
	.addEventListener("click", async function (event) {
		...
		timeActiveElements.length > 0
			? (formCadastroNovoTime.style.pointerEvents = "none")
			: (formCadastroNovoTime.style.pointerEvents = "auto");
	});

// Evento quando o botão "Salvar" de criação é clicado
document
	.querySelector("#saveCampeonatoForm")
	.addEventListener("click", function () {
		...
		if (timeActiveElements.length > 0) {
			...
			axios
				.post(`${url}time_campeonato/enroll`, keys, config)
				.then((response) => {
					Swal.fire({
						icon: "success",
			...
			if (validateForm(formData)) {
				...
				axios
					.post(`${url}time/create`, formData, config)
					.then((response) => {
						Swal.fire({
							icon: "success",
